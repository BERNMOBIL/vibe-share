...
public class UpdateHistoryRepository {
    ...
    public UpdateHistory findLastSuccessUpdate() {
        QueryBuilder.Predicate successStatus = QueryBuilder.Predicate.equals("status", "'" + UpdateManager.Status.SUCCESS + "'");
    ...
    public void insert(Timestamp timestamp) {
        String query = new QueryBuilder.PreparedStatement().Insert("update_history", "time").getQuery();
        jdbcTemplate.update(query, new Object[]{timestamp}, new int[]{Types.TIMESTAMP});
    }

    public void insert(UpdateHistory updateHistory) {
        String query = new QueryBuilder.PreparedStatement().Insert("update_history", "time", "status").getQuery();
